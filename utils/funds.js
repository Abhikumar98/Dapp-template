...
export default function Transactor(localProvider, gasPrice, etherscan) {
	console.log("something", { localProvider });
	if (typeof localProvider !== "undefined") {
		...
		return async (tx, callback) => {
			...
			let network;
			let provider;
			if (ethers.Signer.isSigner(localProvider) === true) {
				provider = localProvider.provider;
				signer = localProvider;
				network =
					localProvider.provider &&
					(await localProvider.provider.getNetwork());
			} else if (localProvider._isProvider) {
			...
			options = {
				...
				transactionHandler: (txInformation) => {
					...
					const possibleFunction =
						callbacks[txInformation.transaction.hash];
			...
			try {
				...
				let message =
					e.data && e.data.message
						...
						? JSON.parse(JSON.parse(JSON.stringify(e.error)).body)
								.error.message
						: e.data
						? e.data
						: JSON.stringify(e);
